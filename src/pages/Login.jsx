...
export default function Login() {
  ...
  return <div className="login-page"><div className="login-frame">
    <header className="login-header"><div className="login-brand"><span className="brand-mark"><HeartPulse size={19}/></span><span>PROAUTH <b>AI</b></span></div></header>
    <main className="login-content">
      ...
      <section className="login-panel">
        ...
        <form onSubmit={submit}>
          <fieldset className="role-picker"><div className="role-grid">{roles.map(({ id, label, detail, icon: Icon }) => <button type="button" key={id} className={`role-option ${role === id ? 'selected' : ''}`} onClick={() => setRole(id)}><span className="role-option-icon"><Icon size={18}/></span><span><strong>{label}</strong><small>{detail}</small></span><i>{role === id && <Check size={12}/>}</i></button>)}</div></fieldset>
        ...
        <div className="demo-credentials"><div><ShieldCheck size={15}/><strong>Demo credentials</strong></div><p>Use <b>demo1234</b> for any account below.</p><div className="credential-list">{roles.map(({ id, label }) => <span key={id}><b>{label}</b><small>{id}@proauth.ai</small></span>)}</div></div>
        <p className="login-notice"><ShieldCheck size={14}/>Demo authentication only. No real backend or password security is used.</p>
      </section>
    </main>
  </div></div>;
}
