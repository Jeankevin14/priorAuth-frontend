...
import { Check, ChevronLeft, ChevronRight, FileText, UploadCloud } from 'lucide-react';
...
export default function NursePatientDetail() {
  ...
  const patient = patientService.getPatientByIdFromList(requests, id);
  ...
  const handleApprove = () => { decisionService.approve(latest.id); navigate('/nurse/review'); };
  const handleRequestInfo = () => { decisionService.requestMoreInformation(latest.id); navigate('/nurse/review'); };
  ...
  return <AppLayout><div className="page simple">
    ...
    <Card><div className="section-title"><div><p className="eyebrow">CLINICAL DECISION</p><h3>{latest.id} · {latest.service}</h3></div><Badge type={latest.tone}>{latest.status}</Badge></div>
      ...
      <div style={{ display: 'flex', gap: 10, marginTop: 14 }}>
        <button className="button primary" type="button" onClick={handleApprove}><Check size={16}/>Approve</button>
        <button className="button" type="button" onClick={handleRequestInfo}><UploadCloud size={16}/>Request More Information</button>
      </div>
    </Card>
