...
export function Field({ label, placeholder, wide, type = 'text', value, onChange }) {
  return <label className={`field ${wide ? 'wide' : ''}`}><span>{label}</span><input type={type} placeholder={placeholder} value={value ?? ''} onChange={onChange}/></label>;
}
